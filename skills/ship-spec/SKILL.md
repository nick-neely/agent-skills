...
can't read session usage itself); see [reference/resume.md](reference/resume.md).
If invoked with `--resume`, skip discovery-from-scratch and follow
reference/resume.md to reconstruct state and continue.

## 1. Discover and plan

Delegate discovery to an `assistant`. Brief it to:

- Run `node ~/.claude/skills/ship-spec/scripts/discover.mjs $1`. That script only
...
- **Parallel lanes:** issues with no blocker between them **and** low file overlap
  ...
  parallelize, read [reference/parallel.md](reference/parallel.md) — it owns the
...
Update the continuity ledger at each transition (see reference/resume.md) so an
